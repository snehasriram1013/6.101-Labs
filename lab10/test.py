...
def mix_precedence(sym, expected, include_pow=False):
    ...
    if include_pow:
        cwrapL = {c: c.wrap_left_at_same_precedence for c in cn if lab.BinOp in c.__bases__}
    ...
    for wrap_val in [True, False]:
        for c in cn:
            ...
            if include_pow and c in cwrapL:
                c.wrap_left_at_same_precedence = wrap_val
    ...
    for c in cn:
        ...
        if include_pow and c in cwrapL:
            c.wrap_left_at_same_precedence = cwrapL[c]
    lab.type = type
    ...
    if no_paren not in bad_strs:
        assert (
            False
        ), "Your code got the expected result for __str__ but is type checking somewhere."
...
def test_repr():
    ...
    assert symbol_rep(safe_eval(repr(exp))) == symbol_rep(safe_eval(expected))
...
def test_style_attributes():
    ...
    for x in objs:
        ...
        for att in dir(x):
            assert (
                getattr(x, att) != name
            ), f"Unexpected name attribute {att} stored in class {name}"
    ...
    for c, o in [(lab.Add, "+"), (lab.Sub, "-"), (lab.Mul, "*"), (lab.Div, "/")]:
        ...
        ), f"Expected {n} to store operand {o} as class attribute!"
...
def test_eval_01():
    ...
    assert (
        lab.Mul.eval == lab.Div.eval == lab.Add.eval == lab.Sub.eval == lab.BinOp.eval
    ), f"Correct but avoid unnecessary repetition!"
...
def test_deriv_00():
    ...
    assert (
        lab.Mul.deriv != lab.Div.deriv
    ), f"Correct functionality but avoid unnecessary type checking / repetition!"
...
def test_simplify_02():
    ...
    assert symbol_rep(result.simplify()) == symbol_rep(expected)
    assert (
        lab.Num.simplify == lab.Var.simplify
    ), f"Correct functionality but avoid unnecessary repetition!"
    assert symbol_rep(result) == symbol_rep(
        result_copy
    ), f"Input to simplify should not be mutated!"
...
def test_pow():
    ...
    result_copy = read_expected("139_in.pyobj")
    result = read_expected("139_in.pyobj")
    expected = lab.Num(1)
    assert symbol_rep(result.simplify()) == symbol_rep(expected)
    assert symbol_rep(result) == symbol_rep(
        result_copy
    ), f"Do not mutate inputs to simplify!"

    result_copy = lab.Pow(lab.Var("x"), lab.Num(0))
    result = lab.Pow(lab.Var("x"), lab.Num(0))
    expected = lab.Num(1)
    assert symbol_rep(result.simplify()) == symbol_rep(expected)
    assert symbol_rep(result) == symbol_rep(
        result_copy
    ...
    result_copy = lab.Pow(lab.Num(0), lab.Var("x"))
    result = lab.Pow(lab.Num(0), lab.Var("x"))
    expected = lab.Num(0)
    assert symbol_rep(result.simplify()) == symbol_rep(expected)
    assert symbol_rep(result) == symbol_rep(
        result_copy
    ...
    result = lab.Pow(lab.Var("x"), lab.Pow(lab.Var("y"), lab.Var("z")))
    expected = lab.Pow(lab.Var("x"), lab.Pow(lab.Var("y"), lab.Var("z")))
    assert symbol_rep(result.simplify()) == symbol_rep(expected)

    result_copy = read_expected("143_in.pyobj")
    result = read_expected("143_in.pyobj")
    ...
    result_copy = read_expected("144_in.pyobj")
    result = read_expected("144_in.pyobj")
    ...
    result_copy = read_expected("145_in.pyobj")
    result = read_expected("145_in.pyobj")
    ...
    result_copy = read_expected("146_in.pyobj")
    result = read_expected("146_in.pyobj")
...
def test_pow_attributes():
    ...
    assert any(
        getattr(x, att) == "**" for att in dir(x)
    ), f"Expected Pow to store operand ** as class attribute!"
    ...
    for c in [
        ...
        if c not in {lab.Var, lab.Num}:
            assert isinstance(c.wrap_right_at_same_precedence, bool)
        if c not in {lab.Var, lab.Num}:
            assert isinstance(c.wrap_left_at_same_precedence, bool)
...
for i in ("Add", "Sub", "Mul", "Div", "Var", "Num", "Pow"):
